...
def get_single_book(path):
    """ get single file
    use user_input later
    """
    book_path = os.path.join(path, 'The_Fall_of_the_House_of_Usher.txt')
    return book_path
...
def lord_func():
    ...
    """
    library_path = '/home/chris/proj/ari/books/'
    # books = get_books(library_path)
    pbook = get_single_book(library_path)
    # the_book.open_book().clean_text().prosses_text().get_readability()
    new_book = Book('fall of usher', pbook)
    new_book.open_book()
    new_book.clean_text()
    new_book.prosses_text()
    print(type(new_book.content))
    end_book = new_book.get_readability()
